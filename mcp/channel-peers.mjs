// Bridge-specific endpoint selection around the official Claude Channels
// notification/reply contract (see channel.mjs). Credentials stay in a private
// local configuration file; discovery exposes only verified routing metadata.
import {open,constants} from 'node:fs/promises';
import {isAbsolute,join} from 'node:path';
import {homedir} from 'node:os';
import {validateForwardTo} from './channel.mjs';

// Used when BRIDGE_CHANNEL_PEERS_FILE is unset, so both hosts can share one
...
export async function loadChannelPeers(path=process.env.BRIDGE_CHANNEL_PEERS_FILE){
  ...
  catch(error){if(!explicit&&error.code==='ENOENT')return [];throw error;}
  try{
    const info=await handle.stat();
    if(!info.isFile() || (info.mode&0o077) || (process.getuid && info.uid!==process.getuid()))throw Error('Channel peers file must be private and owned by this user');
    if(info.size>131072)throw Error('Channel peers configuration too large');
    let peers;
    try{peers=JSON.parse(await handle.readFile('utf8'));}catch{throw Error('Invalid channel peers JSON');}
...
export function validateChannelPeers(peers){
    ...
    for(const p of peers){
      ...
      if(p.title!==undefined&&typeof p.title!=='string')throw Error('Invalid peer title');
    }
    return peers;
}

export class ChannelPeers {
  constructor(load=loadChannelPeers){this.load=load;}
  async identity(peer,timeoutMs=3000){
    const response=await fetch(new URL('/identity',peer.url),{headers:{Authorization:`Bearer ${peer.token}`},
    ...
    return {peerId:peer.id,sessionId:peer.sessionId,title:peer.title??peer.sessionId,app:'claude',
      ...(identity.asyncReplies===true?{asyncReplies:true}:{}),uiVerified:false};
  }
  async discover(){
    ...
    return {candidates:results.flatMap(r=>r.status==='fulfilled'?[r.value]:[]),
      ...
      selectionRequired:true,configured:peers.length>0};
  }
  ...
  async send({peerId,sessionId,body,forwardTo,timeoutMs=180000}){
    const peers=await this.load();const peer=peers.find(p=>p.id===peerId);
    ...
    await this.identity(peer,Math.min(timeoutMs,3000));
    const echo={peerId,sessionId,...(target?{forwardTo:target}:{})};
    // Recheck the configured session at POST time on the receiver as well.
    // Never follow redirects, retry POST, or switch transport on an error.
    try{
      const response=await fetch(new URL('/messages',peer.url),{method:'POST',
        ...
        body:JSON.stringify({sessionId,body,timeoutMs,...(target?{forwardTo:target}:{})}),redirect:'error',signal:AbortSignal.timeout(timeoutMs+5000)});
      ...
      // Avoid reflecting URLs/credentials from arbitrary transport errors.
  ...
  async submit({peerId,sessionId,body,forwardTo,timeoutMs=1800000}){
    const peers=await this.load();const peer=peers.find(p=>p.id===peerId);
    if(!peer||peer.sessionId!==sessionId)throw Error('Unknown or changed destination');
    if(typeof body!=='string'||!body.trim())throw Error('Empty message');
  ...
  async wait({peerId,sessionId,messageId,timeoutMs=1000}){
    const peers=await this.load();const peer=peers.find(p=>p.id===peerId);
    if(!peer||peer.sessionId!==sessionId)throw Error('Unknown or changed destination');
    ...
    try{
      ...
      if(!response.ok||result.sessionId!==sessionId||result.messageId!==messageId||(!validFinal&&!validPending)||
        (result.status==='replied'&&typeof result.reply!=='string'))throw Error('Invalid observation response');
      return {...result,...echo,uiVerified:false};
    }catch{
  ...
  async reply({peerId,sessionId,messageId,body,timeoutMs=10000}){
    const peers=await this.load();const peer=peers.find(p=>p.id===peerId);
    if(!peer||peer.sessionId!==sessionId)throw Error('Unknown or changed destination');
    if(typeof messageId!=='string'||!messageId.trim())throw Error('messageId required');
